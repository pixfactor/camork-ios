# Camork v1 Core — Plan A: 프로젝트 셋업 + DesignSystem (v2)

> **개정 2026-05-19**: momus 리뷰 (Critical 9 + Should-fix 7) 전부 반영.
> 부록 §A에 momus 지적 ↔ 반영 위치 매핑.
...
- `swift test` 또는 Xcode 테스트 패스 (의미 있는 단위 테스트 — toy 아님)
...
**이 plan은 destination을 `iPhone 16 Pro`로 통일.** `iPhone 15 Pro`만 있다면 다음 sed로 plan 본문 일괄 치환:

```bash
sed -i '' 's/iPhone 16 Pro/iPhone 15 Pro/g' docs/superpowers/plans/2026-05-19-camork-v1-core-A-setup.md
```

또는 셸 변수: `export CAMORK_SIM="iPhone 15 Pro"` 후 명령에서 `name=$CAMORK_SIM` 치환 (모든 xcodebuild 명령에 일관 적용 — 잊지 말 것).

- [ ] **Step 3: 현재 `.gitignore` 검증 + 보강** (momus S1 + 재검증 보강 4)
...
- **`Camork/Info.plist`** 라인 보임 (XcodeGen이 자동 생성하는 산출물 — commit 시 매번 diff 발생). 없으면 `.gitignore` 끝에 다음 추가 후 단독 commit:
...
`xcodegen generate` 후 빌드 명령으로 컴파일만 확인:
```bash
xcodegen generate && \
xcodebuild -scheme Camork \
  ...
  build 2>&1 | tail -10
```
Expected: 다른 파일이 없어 still fail. CamorkApp/RootTabView 추가 후 통과 검증.
...
git commit -m "feat(setup): XcodeGen + RootTabView 3개 placeholder + 가림막 hook

- project.yml: iOS 17+, iPhone only, 시스템 appearance 따름
- Asset Catalog: AccentColor 라이트/다크 명시, AppIcon 슬롯
- CamorkApp + RootTabView (3 탭 + appBackgroundShield modifier)
- placeholder 3종: 카메라/현장/설정 (LocalizedStringKey)
- AppBackgroundShield 빈 hook (Plan E에서 구현)
...
git commit -m "feat(design): Spacing/CornerRadius 토큰 (8pt 그리드)"
```
...
struct ColorsTests {
    ...
    func accentDiffersBetweenAppearances() {
        // Asset Catalog의 AccentColor를 trait collection으로 분해해서 비교
        let light = UIColor(named: "AccentColor")?.resolvedColor(
            with: UITraitCollection(userInterfaceStyle: .light)
        )
        let dark = UIColor(named: "AccentColor")?.resolvedColor(
...
git commit -m "feat(design): semantic color 매핑 + AccentColor trait 검증

- Color.camorkAccent / Background / Secondary / Tertiary / Separator / Fill
- ColorsTests: UIColor.resolvedColor 로 라이트≠다크 AccentColor 검증"
```
...
- [ ] **Step 1: 실패 테스트** (로직 검증 가능하도록 static 분기 분리)
...
struct CamorkButtonStyleTests {
    @Test("primary는 borderedProminent + accent tint")
    func primary() {
        let resolved = CamorkButton.resolveStyle(.primary)
        #expect(resolved.isProminent == true)
        #expect(resolved.tint == .camorkAccent)
    }

    @Test("secondary는 bordered + secondary tint (HIG: borderedProminent 1개 원칙)")
    func secondary() {
        let resolved = CamorkButton.resolveStyle(.secondary)
        #expect(resolved.isProminent == false)
        #expect(resolved.tint == nil)   // 시스템 기본 tint
    }

    @Test("destructive는 borderedProminent + red tint")
    func destructive() {
...
struct CamorkButton: View {
    ...
    static func resolveStyle(_ role: Role) -> ResolvedStyle {
        switch role {
        case .primary:
            return ResolvedStyle(isProminent: true, tint: .camorkAccent)
        case .secondary:
            return ResolvedStyle(isProminent: false, tint: nil)
        case .destructive:
            return ResolvedStyle(isProminent: true, tint: .red)
        }
    }
}
...
git commit -m "feat(design): CamorkButton + role별 스타일 분기 + 단위 테스트

- primary: borderedProminent + accent
- secondary: bordered (HIG: borderedProminent 1개 원칙)
- destructive: borderedProminent + red
- resolveStyle 로직 분리로 단위 테스트 가능"
```
...
git commit -m "feat(design): CamorkCard 통일 컨테이너 (시각 검증은 Preview)"
```
...
```json
{
  ...
  "strings" : {
    ...
    "placeholder_camera_description" : {
      "extractionState" : "manual",
      "localizations" : {
        "en" : { "stringUnit" : { "state" : "translated", "value" : "Implementing in Plan B." } },
        "ko" : { "stringUnit" : { "state" : "translated", "value" : "Plan B에서 구현 예정" } }
      }
    },
    "placeholder_camera_title" : {
      "extractionState" : "manual",
    ...
    "placeholder_gallery_description" : {
      "extractionState" : "manual",
      "localizations" : {
        "en" : { "stringUnit" : { "state" : "translated", "value" : "Implementing in Plan C." } },
        "ko" : { "stringUnit" : { "state" : "translated", "value" : "Plan C에서 구현 예정" } }
      }
    },
    "placeholder_gallery_title" : {
      "extractionState" : "manual",
    ...
    "placeholder_settings_description" : {
      "extractionState" : "manual",
      "localizations" : {
        "en" : { "stringUnit" : { "state" : "translated", "value" : "Implementing in Plan E." } },
        "ko" : { "stringUnit" : { "state" : "translated", "value" : "Plan E에서 구현 예정" } }
      }
    },
    "placeholder_settings_title" : {
      "extractionState" : "manual",
...
git commit -m "feat(i18n): String Catalog 한/영 (sourceLanguage: ko)

- 3개 탭 라벨, 3개 placeholder 타이틀/설명, 3개 버튼 라벨
- JSON 직접 작성 (Xcode GUI 비의존)"
```
...
## 다음 단계
Plan B (Storage + Sessions + Camera) 작성으로 진입. 위 5개 인계 항목을 brainstorming/시작 시 정렬.
```
...
git commit -m "docs: Plan A 완료 보고서 + Plan B 인계 메모"
```

...
- **Xcode (Task 0에서 버전 확인) + iOS 17 deployment target + 시뮬레이터(`$CAMORK_SIM`, 기본 `iPhone 17 Pro`)** (Task 0에서 검증). 현재 환경(2026-05): Xcode 26.x / iOS 26.x 시뮬레이터 기준 — iOS 17 deployment target은 iOS 26 시뮬레이터에서 정상 빌드/실행됨.
...
- [ ] **Step 2: 시뮬레이터 가용성 확인 + 환경변수 설정** (momus C8)

```bash
xcrun simctl list devices available | grep "iPhone "
```
Expected: 가용한 iPhone 시뮬레이터 한 줄 이상. 없으면 Xcode → Settings → Platforms에서 다운로드.

**이 plan은 destination을 환경변수 `CAMORK_SIM`으로 통일.** 셸에서 가용 기기 이름으로 export 후 plan의 모든 xcodebuild 명령이 `name=$CAMORK_SIM`을 사용:

```bash
export CAMORK_SIM="iPhone 17 Pro"   # 또는 가용한 다른 iPhone 모델
```

**plan 문서 자체를 sed로 치환하지 말 것** — git 히스토리에 환경 차이가 흔적으로 남음. 환경변수만 셸 세션에 둠.
...
options:
  ...
  deploymentTarget:
    iOS: "17.0"
  # xcodeVersion 의도적 미설정 — Xcode 26.x 환경에서 16.0 강제는 호환 안 됨.
  # XcodeGen이 현재 설치된 Xcode 기준으로 동작.
  createIntermediateGroups: true

settings:
  base:
    SWIFT_VERSION: "5.0"          # Xcode build setting = Swift language mode (5.0 또는 6.0)
...
`$CAMORK_SIM` 시뮬레이터 (예: iPhone 17 Pro)에서 ⌘R 후:
...
**모든 xcodebuild 명령은 `$CAMORK_SIM` 환경변수에 의존.** 셸 세션에서 `export CAMORK_SIM="iPhone 17 Pro"` (또는 가용 기기)를 먼저 실행. `xcrun simctl list devices available | grep "iPhone "`로 가용 기기 확인.
...
| **C8** 시뮬레이터 가용성 | Task 0 Step 2 — 사전 확인 + destination을 `$CAMORK_SIM` 환경변수로 통일 (기본 `iPhone 17 Pro`) |
...
### v3 환경 정합 errata (Xcode 26 / iPhone 17 시리즈 환경)

| 항목 | 반영 위치 |
|---|---|
| `xcodeVersion: "16.0"` 강제 → Xcode 26 환경과 충돌 | `project.yml`에서 라인 제거. XcodeGen이 현재 설치된 Xcode 기준 동작 |
